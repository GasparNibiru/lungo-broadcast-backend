...
async function savePayment(event, payment) {
  ...
  let { data: subscription } = await supabase.from('subscriptions').select('id').eq('asaas_subscription_id', payment.subscription).maybeSingle();
  if (!subscription) {
    const remoteSubscription = await require('../services/asaas').request(`/subscriptions/${encodeURIComponent(payment.subscription)}`);
    if (remoteSubscription?.externalReference) {
      const linked = await supabase.from('subscriptions').update({
        asaas_subscription_id: remoteSubscription.id,
        asaas_status: remoteSubscription.status || 'ACTIVE',
        asaas_sync_status: 'synced',
        asaas_last_error: null,
        asaas_synced_at: new Date().toISOString()
      }).eq('id', remoteSubscription.externalReference).select('id').maybeSingle();
      if (linked.error) throw linked.error;
      subscription = linked.data;
    }
  }
...
async function saveSubscription(event, subscription) {
  ...
  let query = supabase.from('subscriptions').update({ asaas_subscription_id: subscription.id, asaas_status: subscription.status || event.replace('SUBSCRIPTION_', ''), asaas_sync_status: 'synced', asaas_last_error: null, asaas_synced_at: new Date().toISOString() });
  query = subscription.externalReference ? query.eq('id', subscription.externalReference) : query.eq('asaas_subscription_id', subscription.id);
  const result = await query;
  if (result.error) throw result.error;
}

async function saveCheckout(event, checkout) {
  if (!checkout?.id) return;
  const status = checkout.status || event.replace('CHECKOUT_', '');
  let query = supabase.from('subscriptions').update({ asaas_checkout_status: status, asaas_synced_at: new Date().toISOString() });
  query = checkout.externalReference ? query.eq('id', checkout.externalReference) : query.eq('asaas_checkout_id', checkout.id);
  const result = await query;
  if (result.error) throw result.error;
}
...
router.post('/api/webhooks/asaas', async (req, res) => {
  ...
  try { if (event.startsWith('PAYMENT_')) await savePayment(event, req.body.payment); if (event.startsWith('SUBSCRIPTION_')) await saveSubscription(event, req.body.subscription); if (event.startsWith('CHECKOUT_')) await saveCheckout(event, req.body.checkout); await supabase.from('asaas_webhook_events').update({ processed_at: new Date().toISOString(), processing_error: null }).eq('id', eventId); return res.status(200).json({ ok: true }); }
