...
router.get('/api/admin/lead-marketplace', requireAdmin, async (_req, res) => {
  try {
    const [settings, users, wallets, leads, purchases, transactions] = await Promise.all([
      ...
      supabase.from('marketplace_purchases').select('*,users!marketplace_purchases_buyer_user_id_fkey(name),marketplace_leads(name,phone,profile,product_interest)').order('purchased_at', { ascending: false }).limit(200),
