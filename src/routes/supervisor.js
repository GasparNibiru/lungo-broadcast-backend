...
router.post('/api/supervisor/brokers/:userId/email/send', async (req, res) => { if (!UUID.test(req.params.userId)) return res.status(400).json({ ok: false, error: 'Corretor inválido.' }); if (Object.keys(req.body || {}).length) return res.status(400).json({ ok: false, error: 'Esta operação não recebe parâmetros.' }); try { return res.status(200).json({ ok: true, emailDelivery: await service.resendSupervisorBrokerEmail(req.accessUser.organizationId, req.params.userId) }); } catch (error) { return sendError(res, error); } });
...
router.delete('/api/supervisor/brokers/:userId', async (req, res) => { if (!UUID.test(req.params.userId)) return res.status(400).json({ ok: false, error: 'Corretor inválido.' }); try { return res.status(200).json({ ok: true, broker: await service.archiveSupervisorBroker(req.accessUser.organizationId, req.params.userId) }); } catch (error) { return sendError(res, error); } });
