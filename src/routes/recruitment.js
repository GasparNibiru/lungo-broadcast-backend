...
router.patch('/api/supervisor/recruitment/candidates/:id', requireAccess('supervisor'), (req, res) => { const data = load(); const item = data.candidates.find((c) => c.id === req.params.id && c.organizationId === req.accessUser.organizationId); if (!item) return res.status(404).json({ ok: false, error: 'Candidato não encontrado.' }); if ('email' in req.body) { const email = typeof req.body.email === 'string' ? req.body.email.trim() : ''; if (!EMAIL.test(email) || email.length > 254) return res.status(400).json({ ok: false, error: 'Informe um e-mail válido.' }); if (item.stage !== 'teste_enviado' || item.disc?.completedAt) return res.status(409).json({ ok: false, error: 'O e-mail só pode ser corrigido enquanto o teste aguarda resposta.' }); item.email = email; } if (req.body.stage) { if (!STAGES.has(req.body.stage)) return res.status(400).json({ ok: false, error: 'Etapa inválida.' }); item.stage = req.body.stage; if (item.stage === 'aprovado') item.approvedAt ||= new Date().toISOString(); } if ('notes' in req.body) item.notes = String(req.body.notes || '').trim().slice(0, 3000); if ('hirePending' in req.body) item.hirePending = Boolean(req.body.hirePending); if ('hiredUserId' in req.body) { const hiredUserId = String(req.body.hiredUserId || '').trim(); if (hiredUserId && item.stage !== 'aprovado') return res.status(409).json({ ok: false, error: 'Aprove o candidato antes de liberar o acesso.' }); item.hiredUserId = hiredUserId; if (hiredUserId) { item.accessGrantedAt ||= new Date().toISOString(); item.hirePending = false; } } if (req.body.seen) item.seenAt ||= new Date().toISOString(); item.updatedAt = new Date().toISOString(); save(data); res.json({ ok: true, candidate: item }); });
