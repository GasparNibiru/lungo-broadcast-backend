// Admin Master Panel v3 for Lungo Corretores.
// One self-contained module for dashboard, token generation, deactivate/reactivate and Evolution instance deletion.
...
const VERSION = '3.0.0-admin-master-actions-fixed';
...
async function setActiveRoute(req, res, active) {
  try {
    ...
    if (active && clients[index].instanceStatus === 'deleted') clients[index].instanceStatus = 'deleted';
    saveArray(CLIENTS_FILE, clients);
...
async function deleteInstanceRoute(req, res) {
  try {
    ...
    clients[index].instanceDeleteError = evolution.ok ? '' : (evolution.reason || 'evolution_delete_not_confirmed');
    clients[index].updatedAt = now;
    saveArray(CLIENTS_FILE, clients);
    return send(res, 200, { ok: true, localMarked: true, evolution, client: publicClient(clients[index]), dashboard: buildDashboard(), version: VERSION });
...
function register(app) {
  ...
  registered = true;
  app.options('/api/admin-master-v3/health', (req, res) => send(res, 204, {}));
  app.options('/api/admin-master-v3/login', (req, res) => send(res, 204, {}));
  app.options('/api/admin-master-v3/dashboard', (req, res) => send(res, 204, {}));
  app.options('/api/admin-master-v3/clients', (req, res) => send(res, 204, {}));
  app.options('/api/admin-master-v3/clients/:token/deactivate', (req, res) => send(res, 204, {}));
  app.options('/api/admin-master-v3/clients/:token/reactivate', (req, res) => send(res, 204, {}));
  app.options('/api/admin-master-v3/clients/:token/delete-instance', (req, res) => send(res, 204, {}));
  app.get('/api/admin-master-v3/health', (req, res) => send(res, 200, { ok: true, module: 'admin-master-v3', version: VERSION, corretorAppUrl: DEFAULT_CORRETOR_APP_URL, evolutionConfigured: Boolean(evolutionBaseUrl() && process.env.EVOLUTION_API_KEY), deletePath: process.env.EVOLUTION_DELETE_INSTANCE_PATH || '/instance/delete/:instanceName', clientsFile: CLIENTS_FILE, leadsFile: LEADS_FILE, customerClientsFile: CUSTOMER_CLIENTS_FILE, time: new Date().toISOString() }));
  app.post('/api/admin-master-v3/login', loginRoute);
  app.get('/api/admin-master-v3/dashboard', dashboardRoute);
  app.post('/api/admin-master-v3/clients', createClientRoute);
  app.post('/api/admin-master-v3/clients/:token/deactivate', (req, res) => setActiveRoute(req, res, false));
  app.post('/api/admin-master-v3/clients/:token/reactivate', (req, res) => setActiveRoute(req, res, true));
  app.post('/api/admin-master-v3/clients/:token/delete-instance', deleteInstanceRoute);
}
