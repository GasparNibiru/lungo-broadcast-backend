...
test('versioned completion preserves existing tests, requires commercial answers for new ones and scopes read receipts',async()=>{
 ...
 require.cache[mailPath]={id:mailPath,filename:mailPath,loaded:true,exports:{sendRecruitmentDismissalEmail:async()=>{dismissalMails++;return {sent:true};},sendRecruitmentEmail:()=>new Promise(resolve=>{releaseMail=()=>resolve({messageId:'mock'});})}};
 ...
 const supervisorPath=require.resolve('../src/services/supervisor'),oldSupervisor=require.cache[supervisorPath];require.cache[supervisorPath]={id:supervisorPath,filename:supervisorPath,loaded:true,exports:{archiveSupervisorBroker:async()=>{archivals++;}}};
